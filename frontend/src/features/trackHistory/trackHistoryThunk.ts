...
export const fetchTrackHistory = createAsyncThunk<TrackHistory[], void,{ state: RootState }>(
  "tracksHistory/fetchTrackHistory ",
    ...
    const tracksResponse = await axiosApi<TrackHistory[]>("/track_history",{headers:{Authorization: token, "Content-Type": "application/json"},});
...
>(
    ...
    async (TrackHistoryPost, {rejectWithValue}) => {
        try{
        await axiosApi.post("/track_history", {track:TrackHistoryPost.Id},{headers:{Authorization: TrackHistoryPost.token}});
        }catch(error){
