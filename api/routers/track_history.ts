...
trackHistoryRouter.post('/', async (req, res, next) => {
    ...
    if (req.body.track) {
        ...
        if (!track) res.status(404).send({error:'Not Found album'});
        artist = await Artist.findById({_id: album?.artist});}

    const newTrackHistory = {
        user: user._id,
        track: req.body.track,
        artistName: artist? artist?.name: "anonimus",
    };
...
trackHistoryRouter.get('/', auth, async (req, res, next) => {
    try {
        ...
        const trackHistory = await TrackHistory.find({user: user}).populate("track").sort({datetime: -1});
        res.send(trackHistory);
    } catch (e) {
        next(e);
    }
